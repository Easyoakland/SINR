...
impl Ptr {
    // ---
    // TODO the first slot is not valid target of `Ptr` because it represents these special values
    // ---
...
impl Net {
    ...
    /// Interact an active pair where the `right` `Ptr`'s target is not a primary port and instead is either a redirector or an auxillary port.
